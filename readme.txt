...
I have avoided integrating additional packages - I ran this code at regular intervals through Windows Task Scheduler on a local machine with average specs.
...
I have also included the .bat code used in conjunction with Task Scheduler. The .bat file is quite basic, as my familiarity is limited. 
...
This code is used to boot two fields into a secondary unique identifier field. This came at the request of folks looking to assign client name to the custom record label AND a secondary unique field, while also avoiding an interrupted user experience if an external party submitted a duplicate client from the survey view. There are surely better ways to handle this scenario within REDCap, but this is how it was approached, and the concept can be applied elsewhere.

REDCapR Usage:

redcap_report is used here is to avoid system burden as redcap_read can take quite some time to pull on smaller data sets, and we are able to skip filtering within R itself. Similarly, we use redcap_write_oneshot, as this code runs at an interval of every fifteen minutes. At a light volume of exported records, it seems rare for oneshots to timeout. You will be notified if you are trying to import a duplicate record.
